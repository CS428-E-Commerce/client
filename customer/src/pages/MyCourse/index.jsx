  ...
  <div className={classes.classList}>
    {classData.map((classItem, index) => (
      <React.Fragment key={classItem.id}>
        ...
        {index !== classData.length - 1 && <div className={classes.divider} />}
      </React.Fragment>
    ))}
  </div>
          ...
          <div className={classes.detailColumn}>
            <div className={classes.attendeesSection}>
              ...
              <div className={classes.attendeesList}>
                {classData[selectedClass - 1].attendees.map((attendee, index) => (
                  <div className={classes.attendee} key={index}>
                    <img src={attendee.avatarUrl} alt={`Avatar ${attendee.name}`} />
                    <p>{attendee.name}</p>
                  </div>
                ))}
              </div>
            ...
            <div className={classes.tutorInfoSection}>
              <div className={classes.tutorInfoRow}>
                <img src={classData[selectedClass - 1].tutor.avatarUrl} alt={`Avatar ${classData[selectedClass - 1].tutor.name}`} />
                <p>Taught by {classData[selectedClass - 1].tutor.name}</p>
              </div>
              <p>On Vinglish since {classData[selectedClass - 1].tutor.since}</p>
              <button className={classes.contactButton}>Contact the Tutor</button>
              {classData[selectedClass - 1].tutor.verified && (
                <div className={classes.verifiedRow}>
                  <img src={"https://static.vecteezy.com/system/resources/previews/013/743/787/original/check-mark-shield-icon-png.png"} alt="Verified Icon" />
                  <p>Verified Tutor</p>
                </div>
              )}
              <div className={classes.reviewRow}>
                <img src={"https://cdn-icons-png.flaticon.com/512/541/541415.png"} alt="Star Icon" />
                <p>{classData[selectedClass - 1].tutor.rating} stars</p>
                <p>({classData[selectedClass - 1].tutor.reviewCount} reviews)</p>
              </div>
              <div className={classes.lessonsRow}>
                <img src={"https://cdn-icons-png.flaticon.com/512/4344/4344806.png"} alt="Graduating Cap Icon" />
                <p>{classData[selectedClass - 1].tutor.lessonsTaught} lessons taught</p>
              </div>
            </div>
          </div>
            <div className={classes.upcomingSessionsSection}>
            <>Your Upcoming Class:</>
            {classData[selectedClass - 1].upcomingSessions.map((session, index) => (
              <div key={index} className={classes.upcomingCard}>
                <img src={classData[selectedClass - 1].imageUrl} alt={`Class ${classData[selectedClass - 1].title}`} />
                <div className={classes["card-content"]}>
                  <h2>{session}</h2>
                  <p className={classes["class-detail"]}>{classData[selectedClass - 1].detail}</p>
                </div>
                ...
                <div className={classes["calendar-row"]}>
                  <div className={classes.calendarButton}>
                    <img src={'https://cdn-icons-png.flaticon.com/512/55/55281.png'} alt="Calendar Icon" />
                    <span>Add to your calendar</span>
                  </div>
                  <div className={classes.inviteButton}>
                    <img src={'https://cdn-icons-png.flaticon.com/512/4458/4458537.png'} alt="Invite Icon" />
